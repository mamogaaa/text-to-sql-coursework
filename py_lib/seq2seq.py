...
def TRAIN(train_dataset):
    ...
    def readLangs(lang1, lang2, reverse=False):
        ...
        pairs = list(
            zip(
                train_dataset.get_preprocessed_queries(),
                train_dataset.get_sql()
            )
        )
    ...
    def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
        ...
        if not os.path.isfile('encoder.dict') or not os.path.isfile('decoder.dict'):
    ...
    loss, acc_train, acc_validation = trainIters(encoder1, attn_decoder1, 75000, print_every=20, learning_rate=0.04)
    ...
    def EVALUATE(text):
        res = evaluate(encoder1, attn_decoder1, text)[0]
        return " ".join(filter_string(" ".join(res)))
